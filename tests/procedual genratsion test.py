...
def update_grid(grid):
    ...
    for y in range(height):
        for x in range(width):
            ...
            # game of life rules - a cell stays alive if it has 2 or 3 neigbors
            # becomes dead if it has exactly 3 neigbors
            # this creats a completable path
     ...
     for event in pygame.event.get():
        ...
        for y in range(height):
            for x in range(width):
                ...
                pygame.draw.rect(screen, color, (x * 1, y * 1, cell_size, cell_size))
